...
from common import clean_text, emoji_re, load_csv, wxid_re
from config import DATA_DIR, FONT_EMOJI, FONT_MAIN, GROUPS, MIN_DF, N_TOPICS
...
def init_matplotlib():
    """中文字体（加 emoji 回退）；字体文件缺失时跳过，不阻断。"""
    for fp in (FONT_MAIN, FONT_EMOJI):
...
def pie(name, rows):
    ...
    ax.set_title(f"“{name}”群 成员发言条数占比\n总计 {total:,} 条 · {tmin[:10]} ~ {tmax[:10]} · 发言成员 {len(items)} 人",
                 fontsize=15, pad=18)
    ax.axis("equal")
    out = os.path.join(DATA_DIR, f"{name}_pie.png")
...
def main():
    ...
    init_matplotlib()
    for gname in GROUPS:
        rows = load_csv(os.path.join(DATA_DIR, f"{gname}.csv"))
        ...
        with open(os.path.join(DATA_DIR, f"{gname}_stats.json"), "w", encoding="utf-8") as f:
